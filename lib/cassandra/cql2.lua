...
function _M_buf.read(buf, n_bytes)
    if n_bytes and n_bytes <= 0 then
        error("must read a positive number of bytes", 2)
    end
...
function _M_buf.write(buf, bytes)
    if type(bytes) ~= "string" then
        error("bytes must be a string", 2)
    end
...
function _M_buf.copy(src_buf, dst_buf)
    for i = 1, src_buf.i - 1 do
        _M_buf.write(dst_buf, src_buf.t[i])
    end
end
...
function _M_buf.read_value(buf)
    ...
    error("n component of [value] notation is invalid: " .. n)
end
...
local function marsh_inet(value)
    ...
    if not find(value, ".", nil, true) then
        -- ipv6
        buf_t = new_tab(16 + 1, 0) -- +1: [int] port

        local port
        local port_idx = find(value, "]", nil, true)
        if port_idx then
            local port_str = sub(value, port_idx + 2)
            port           = marsh_int(tonumber(port_str))
            value          = sub(value, 2, port_idx - 1)
        end
        ...
        local port_idx = find(value, ":")
        if port_idx then
            local port_str = sub(value, port_idx + 1)
            buf_t[n]       = marsh_int(tonumber(port_str))
        end
...
function _M_buf.read_inet(buf)
    ...
    if size >= 16 then
        ...
        local ip = concat(t_buf, ":")

        -- port
        if size == 20 then
            local marshed_port = sub(bytes, 17)
            ip = fmt("[%s]:%d", concat(t_buf, ":"), unmarsh_int(marshed_port))
        end
    ...
    -- port
    if size == 8 then
        local marshed_port = sub(bytes, 5)
        ip = ip .. ":" .. unmarsh_int(marshed_port)
    end
